...
impl CssNumber for CssUnsignedNumber
{
	...
	{
		CssUnsignedNumber::new(value).map_err(|cssNumberConversionError| CouldNotParseCssSignedNumber(cssNumberConversionError, value))
	}
}
...
impl Unit for CssUnsignedNumber
{
	...
	fn from_raw_css_for_var_expression_evaluation(value: &str, _is_not_in_page_rule: bool) -> Option<Self>
	{
		fn from_raw_css_for_var_expression_evaluation_internal<'i: 't, 't>(input: &mut Parser<'i, 't>) -> Result<CssUnsignedNumber, ParseError<'i, CustomParseError<'i>>>
		{
			...
			{
				Token::Number { value, int_value, .. } => CssUnsignedNumber::parseNumber(value, int_value).map_err(|error| input.new_custom_error(error)),
